...
async def download_pdf_file(url, filename, max_retries=3):
    """
    Download PDF file using requests with retry logic
    """
    headers = {
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = requests.get(encoded_url, headers=headers, timeout=120, stream=True)
...
async def download_pdf_cloudscraper(url, filename, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = scraper.get(encoded_url, timeout=120)
            ...
            if response.status_code == 200:
                ...
                print(f"CloudScraper Attempt {attempt + 1}: HTTP {response.status_code}")
                
        except Exception as e:
            print(f"CloudScraper Attempt {attempt + 1} failed: {str(e)}")
            await asyncio.sleep(2)
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    try:
                        ...
                        # Check if it's Utkarsh app PDF or similar
                        if any(domain in url for domain in ["utkarshapp.com", "apps-s3-prod", "apps-s3-jw-prod"]):
                            # Use direct download for Utkarsh PDFs
                            ...
                            download_success = await download_pdf_file(url, pdf_filename)
                            
                            if not download_success:
                                # Try with cloudscraper as fallback
                                download_success = await download_pdf_cloudscraper(url, pdf_filename)
                            
                            await prog_msg.delete()
                            
                        elif "cwmediabkt99" in url:
                            # Use cloudscraper for CW media
                            prog_msg = await bot.send_message(
                                channel_id, 
                                f"<i><b>📥 Downloading PDF...</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>",
                                disable_web_page_preview=True
                            ...
                            if not download_success:
                                # Fallback to yt-dlp for other PDFs
                                cmd = f'yt-dlp -o "{pdf_filename}" "{url}"'
                                download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                                os.system(download_cmd)
                                download_success = os.path.exists(pdf_filename) and os.path.getsize(pdf_filename) > 0
                        
                        # Send PDF if download was successful
                        if download_success and os.path.exists(pdf_filename):
                            ...
                            raise Exception("PDF download failed - file not created or empty")
