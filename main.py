...
def sanitize_filename(name):
    ...
    Handles Hindi/Devanagari and other special characters
    """
...
async def download_pdf_file(url, filename, max_retries=5):
    """
    Download PDF file using requests with retry logic and proper handling
    """
    headers_list = [
        {
    ...
    for attempt in range(max_retries):
        try:
            # URL encode the URL properly - handle spaces and special chars
            encoded_url = url.strip()
            encoded_url = encoded_url.replace(" ", "%20")
            
            # Pick headers based on attempt
            headers = headers_list[attempt % len(headers_list)]
            
            print(f"[PDF Download] Attempt {attempt + 1}/{max_retries}")
            print(f"[PDF Download] URL: {encoded_url[:200]}")
            print(f"[PDF Download] Using User-Agent: {headers['User-Agent'][:50]}")
            ...
            print(f"[PDF Download] Content-Type: {response.headers.get('Content-Type', 'unknown')}")
            print(f"[PDF Download] Content-Length: {response.headers.get('Content-Length', 'unknown')}")
            ...
            if response.status_code == 200:
                ...
                print(f"[PDF Download] Downloaded size: {total_size} bytes")
                
                # Verify file was created and has content
                if os.path.exists(filename) and os.path.getsize(filename) > 100:
                    # Check if file is actually a PDF or valid content (not HTML error page)
                    ...
                    if first_bytes.startswith(b'<!DOCTYPE') or first_bytes.startswith(b'<html') or first_bytes.startswith(b'<HTML'):
                        print(f"[PDF Download] Received HTML instead of PDF, retrying...")
                        os.remove(filename)
                        await asyncio.sleep(2)
                        continue
                    
                    print(f"[PDF Download] SUCCESS! File saved: {filename} ({total_size} bytes)")
                    session.close()
                    return True
                else:
                    print(f"[PDF Download] File too small or not created")
                    if os.path.exists(filename):
                        os.remove(filename)
                    await asyncio.sleep(2)
                    continue
            elif response.status_code == 403:
                print(f"[PDF Download] 403 Forbidden - trying different headers")
                await asyncio.sleep(2)
                continue
            elif response.status_code == 404:
                print(f"[PDF Download] 404 Not Found")
                session.close()
                return False
            else:
                print(f"[PDF Download] Attempt {attempt + 1}: HTTP {response.status_code}")
                await asyncio.sleep(2)
                
            session.close()
                
        except requests.exceptions.SSLError as e:
            print(f"[PDF Download] SSL Error on attempt {attempt + 1}: {str(e)}")
            await asyncio.sleep(2)
        except requests.exceptions.ConnectionError as e:
            print(f"[PDF Download] Connection Error on attempt {attempt + 1}: {str(e)}")
            await asyncio.sleep(3)
        except requests.exceptions.Timeout as e:
            print(f"[PDF Download] Timeout on attempt {attempt + 1}: {str(e)}")
            await asyncio.sleep(2)
        except Exception as e:
            print(f"[PDF Download] Attempt {attempt + 1} failed: {type(e).__name__}: {str(e)}")
            await asyncio.sleep(2)
    
    return False


async def download_pdf_aiohttp(url, filename, max_retries=3):
    """
    Download PDF using aiohttp (async) with proper handling
    """
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept': 'application/pdf,application/octet-stream,*/*',
    ...
    for attempt in range(max_retries):
        try:
            encoded_url = url.strip().replace(" ", "%20")
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                async with session.get(encoded_url, headers=headers, allow_redirects=True) as response:
                    print(f"[Aiohttp PDF] Attempt {attempt + 1} - Status: {response.status}")
                    ...
                    if response.status == 200:
                        ...
                        print(f"[Aiohttp PDF] Downloaded: {total_size} bytes")
                        ...
                        if os.path.exists(filename) and os.path.getsize(filename) > 100:
                            ...
                            if first_bytes.startswith(b'<!DOCTYPE') or first_bytes.startswith(b'<html'):
                                print(f"[Aiohttp PDF] Got HTML instead of PDF")
                                os.remove(filename)
                                await asyncio.sleep(2)
                                continue
                        ...
                        print(f"[Aiohttp PDF] HTTP {response.status}")
                        await asyncio.sleep(2)
                        
        except Exception as e:
            print(f"[Aiohttp PDF] Attempt {attempt + 1} failed: {type(e).__name__}: {str(e)}")
            await asyncio.sleep(2)
    
    return False


async def download_pdf_cloudscraper(url, filename, max_retries=3):
    """
    Download PDF using cloudscraper for protected URLs
    """
    os.makedirs(os.path.dirname(filename) if os.path.dirname(filename) else '.', exist_ok=True)
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"[CloudScraper PDF] Attempt {attempt + 1}")
            ...
            print(f"[CloudScraper PDF] Status: {response.status_code}, Size: {len(response.content)}")
            
            if response.status_code == 200 and len(response.content) > 100:
                # Check if content is HTML error page
                if response.content[:20].startswith(b'<!DOCTYPE') or response.content[:20].startswith(b'<html'):
                    print(f"[CloudScraper PDF] Got HTML instead of PDF")
                    await asyncio.sleep(2)
                    continue
                ...
                if os.path.exists(filename) and os.path.getsize(filename) > 100:
                    print(f"[CloudScraper PDF] SUCCESS! Saved: {filename}")
                    return True
                else:
                    if os.path.exists(filename):
                        os.remove(filename)
                ...
                print(f"[CloudScraper PDF] Attempt {attempt + 1}: HTTP {response.status_code}")
                await asyncio.sleep(2)
                
        except Exception as e:
            print(f"[CloudScraper PDF] Attempt {attempt + 1} failed: {type(e).__name__}: {str(e)}")
            await asyncio.sleep(2)
    
    return False


async def download_pdf_wget(url, filename):
    """
    Download PDF using wget as final fallback
    """
    try:
        encoded_url = url.strip().replace(" ", "%20")
        ...
        cmd = f'wget --no-check-certificate -q -O "{filename}" --timeout=120 --tries=3 --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" "{encoded_url}"'
        
        print(f"[Wget PDF] Trying wget download...")
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 100:
            with open(filename, 'rb') as f:
                first_bytes = f.read(20)
            if first_bytes.startswith(b'<!DOCTYPE') or first_bytes.startswith(b'<html'):
                print(f"[Wget PDF] Got HTML instead of PDF")
                os.remove(filename)
                return False
            print(f"[Wget PDF] SUCCESS!")
            return True
        else:
            if os.path.exists(filename):
                os.remove(filename)
        ...
        print(f"[Wget PDF] Failed: {str(e)}")
        return False


async def download_pdf_curl(url, filename):
    """
    Download PDF using curl as another fallback
    """
    try:
        encoded_url = url.strip().replace(" ", "%20")
        ...
        cmd = f'curl -L -k -s -o "{filename}" --max-time 120 --retry 3 -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" "{encoded_url}"'
        
        print(f"[Curl PDF] Trying curl download...")
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 100:
            with open(filename, 'rb') as f:
                first_bytes = f.read(20)
            if first_bytes.startswith(b'<!DOCTYPE') or first_bytes.startswith(b'<html'):
                print(f"[Curl PDF] Got HTML instead of PDF")
                os.remove(filename)
                return False
            print(f"[Curl PDF] SUCCESS!")
            return True
        else:
            if os.path.exists(filename):
                os.remove(filename)
        ...
        print(f"[Curl PDF] Failed: {str(e)}")
        return False


async def download_pdf_master(url, filename):
    """
    Master PDF download function that tries all methods
    """
    print(f"\n{'='*60}")
    print(f"[PDF Master] Starting download for: {url[:150]}")
    print(f"[PDF Master] Target file: {filename}")
    print(f"{'='*60}")
    
    # Method 1: requests with multiple header combinations
    print(f"\n[PDF Master] Method 1: requests library")
    if await download_pdf_file(url, filename):
        print(f"[PDF Master] ✅ Method 1 SUCCESS")
        return True
    ...
    # Method 2: aiohttp async download
    print(f"\n[PDF Master] Method 2: aiohttp")
    if await download_pdf_aiohttp(url, filename):
        print(f"[PDF Master] ✅ Method 2 SUCCESS")
        return True
    ...
    print(f"\n[PDF Master] Method 3: cloudscraper")
    if await download_pdf_cloudscraper(url, filename):
        print(f"[PDF Master] ✅ Method 3 SUCCESS")
        return True
    ...
    print(f"\n[PDF Master] Method 4: curl")
    if await download_pdf_curl(url, filename):
        print(f"[PDF Master] ✅ Method 4 SUCCESS")
        return True
    
    if os.path.exists(filename):
        os.remove(filename)
    
    # Method 5: wget command
    print(f"\n[PDF Master] Method 5: wget")
    if await download_pdf_wget(url, filename):
        print(f"[PDF Master] ✅ Method 5 SUCCESS")
        return True
    
    if os.path.exists(filename):
        os.remove(filename)
    
    # Method 6: yt-dlp as last resort
    print(f"\n[PDF Master] Method 6: yt-dlp")
    try:
        encoded_url = url.strip().replace(" ", "%20")
        cmd = f'yt-dlp --no-check-certificate -o "{filename}" "{encoded_url}" -R 25 --fragment-retries 25'
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 100:
            print(f"[PDF Master] ✅ Method 6 SUCCESS")
            return True
    except Exception as e:
        print(f"[PDF Master] Method 6 failed: {str(e)}")
    
    if os.path.exists(filename):
        os.remove(filename)
    
    print(f"\n[PDF Master] ❌ ALL METHODS FAILED for: {url[:100]}")
    return False
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            safe_name = sanitize_filename(name1[:60])
            
            if "," in raw_text3:
                name = f'{PRENAME} {name1[:60]}'
                safe_filename = f'{sanitize_filename(PRENAME)}_{safe_name}'
            else:
                name = f'{name1[:60]}'
            ...
            if "acecwply" in url:
                cmd = f'yt-dlp -o "{safe_filename}.%(ext)s" -f "bestvideo[height<={raw_text2}]+bestaudio" --hls-prefer-ffmpeg --no-keep-video --remux-video mkv --no-warning "{url}"'
            ...
            if "jw-prod" in url:
                ...
                cmd = f'yt-dlp -o "{safe_filename}.mp4" "{url}"'
            elif "webvideos.classplusapp." in url:
                cmd = f'yt-dlp --add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{safe_filename}.mp4"'
            elif "youtube.com" in url or "youtu.be" in url:
                cmd = f'yt-dlp --cookies youtube_cookies.txt -f "{ytf}" "{url}" -o "{safe_filename}".mp4'
            else:
                cmd = f'yt-dlp -f "{ytf}" "{url}" -o "{safe_filename}.mp4"'
            ...
            try:
                ...
                if "drive" in url:
                    try:
                        ka = await helper.download(url, safe_filename)
                    ...
                    try:
                        # Use safe filename for PDF
                        pdf_filename = f'{path}/{safe_filename}.pdf'
                        ...
                        prog_msg = await bot.send_message(
                            channel_id, 
                            f"<i><b>📥 Downloading PDF...</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>\n\n<i>Trying multiple download methods...</i>",
                            disable_web_page_preview=True
                        )
                        
                        # Use master download function that tries all methods
                        download_success = await download_pdf_master(url, pdf_filename)
                        
                        await prog_msg.delete()
                        
                        # Send PDF if download was successful
                        if download_success and os.path.exists(pdf_filename) and os.path.getsize(pdf_filename) > 100:
                            try:
                                ...
                                try:
                                    ...
                                    raise Exception(f"Send failed after FloodWait: {str(send_err)}")
                            finally:
                                if os.path.exists(pdf_filename):
                                    os.remove(pdf_filename)
                        else:
                            raise Exception(f"All download methods failed for this PDF. URL: {url[:100]}")
                    ...
                    try:
                        html_filename = f"{path}/{safe_filename}.html"
                        await helper.pdf_download(f"{api_url}utkash-ws?url={url}&authorization={api_token}", html_filename)
                        time.sleep(1)
                        await bot.send_document(chat_id=channel_id, document=html_filename, caption=cchtml)
                    ...
                    try:
                        ext = url.split('.')[-1].split('?')[0]  # Handle URL params
                        img_filename = f'{path}/{safe_filename}.{ext}'
                        
                        # Try direct download first
                        download_success = await download_pdf_file(url, img_filename)
                        
                        if not download_success:
                            # Fallback to yt-dlp
                            cmd = f'yt-dlp -o "{img_filename}" "{url}"'
                            download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                            os.system(download_cmd)
                        
                        if os.path.exists(img_filename):
                            copy = await bot.send_photo(chat_id=channel_id, photo=img_filename, caption=ccimg)
                            count += 1
                    ...
                    try:
                        ...
                        audio_filename = f'{path}/{safe_filename}.{ext}'
                        cmd = f'yt-dlp -x --audio-format {ext} -o "{audio_filename}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        
                        if os.path.exists(audio_filename):
                            await bot.send_document(chat_id=channel_id, document=audio_filename, caption=cc1)
                    ...
                    try:
                        res_file = await helper.download_and_decrypt_video(url, cmd, safe_filename, appxkey)  
                        filename = res_file  
                    ...
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    res_file = await helper.decrypt_and_merge_video(mpd, keys_string, path, safe_filename, raw_text2)
                    filename = res_file
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                    count += 1
                    ...
                    Show = f"<i><b>📥 Fast Video Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>"
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    res_file = await helper.download_video(url, cmd, safe_filename)
                    filename = res_file
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                    count += 1
